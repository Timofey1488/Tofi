...
class StatementViewTest(TestCase):
    ...
    def test_statement_view_authenticated(self):
        ...
        # Добавляем платежи
        Payment.objects.create(card=self.card, amount=Decimal('50'), deposit_pending=False)
        Payment.objects.create(card=self.card, amount=Decimal('30'), deposit_pending=True)

        # Отправляем GET-запрос на страницу выписки
        response = self.client.get(reverse('accounts:card_history', kwargs={'card_id': self.card.id}))
    ...
    def test_statement_view_unauthenticated(self):
        ...
        self.assertRedirects(response, '/accounts/login/?next=' + reverse('accounts:card_history', kwargs={'card_id': self.card.id}))