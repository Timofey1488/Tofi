...
from credits.forms import CreditApplicationForm, CreditApprovalForm
from credits.views import calculate_monthly_payment, approve_credit
...
class CreditsTestCase(TestCase):
    ...
    def test_approve_credit_view_post_approval(self):
        ...
        credit_application = CreditApplication.objects.create(user=self.user, amount=1000, status='PENDING')
        data = {'approved': True}  # Одобряем кредитную заявку
        response = self.client.post(reverse('credits:approve_credit', args=[credit_application.id]), data)
        self.assertEqual(response.status_code, 302)
    ...
    def test_approve_credit_view_post_rejection(self):
        ...
        response = self.client.post(reverse('credits:approve_credit', args=[credit_application.id]), data)
        self.assertEqual(response.status_code, 302)  # Ожидаем редирект после успешного отклонения
    ...
    def test_active_credits_view(self):
        ...
        Credit.objects.create(user=self.user, amount=1000, interest_rate=5.0, term_months=12, monthly_payment=86.61,
                              remaining_amount=1000, status='APPROVED')
